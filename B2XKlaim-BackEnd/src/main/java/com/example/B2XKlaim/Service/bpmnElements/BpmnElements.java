    ...
    public BpmnElements(Map<String, BpmnElement> elementsById, Map<String, Set<String>> connections) {
     ...
     public void analyzeInteractions() {
         ...
         for (MessageFLow flow : messageFlows) {
              if (flow == null) continue; // Safety check
             // Use getters generated by Lombok based on field names in MessageFLow
     ...
     private String findParticipantIdByName(String name, List<PL> participantsList) {
         ...
         return participantsList.stream()
                 .filter(Objects::nonNull)
                 .filter(p -> name.equals(p.getName())) // Assumes PL has getName()
                 .map(PL::getId)                       // Assumes PL has getId()
                 .findFirst()
                 .orElse(null);
     }
 
     /**
      * **CRITICAL HELPER - REFLECTION-BASED IMPLEMENTATION:**
      * Determines the ID of the participant that contains the given BPMN element by
      * attempting to call a getProcessId() method on the element via reflection.
      *
      * !! IMPORTANT !!
      * This implementation REQUIRES that relevant BpmnElement classes
      * (SSE, SIC, ST, Tasks, Events etc.) implement a public method:
      * public String getProcessId()
      * and that this method returns the ID of the <bpmn:process> they belong to.
      * This link MUST be established during parsing (e.g., in BpmnElementFactory).
      * If the method doesn't exist or isn't populated, this lookup will fail.
      *
      * @param elementId The ID of the BPMN element (event, task, etc.).
      * @param participantsMap A map of participant ID -> PL object for efficiency.
      * @return The ID of the enclosing participant, or null if not reliably found.
      */
     ...
     public List<BpmnElement> findStartEventsForProcess(String processId) {
         ...
         for (BpmnElement element : elementsById.values()) {
             ...
             if (isStartEvent) {
                 ...
                 // This MUST match the logic used in getParticipantIdForElement
